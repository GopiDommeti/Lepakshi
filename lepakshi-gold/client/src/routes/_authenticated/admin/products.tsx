  ...
  const duplicate = useMutation({
    mutationFn: async (p: AdminProduct) => {
      ...
      if (p.variations.length > 0) {
        const { error: vErr } = await db.from("variations").insert(
          p.variations.map((v) => ({
  ...
  const save = useMutation({
    mutationFn: async (d: ProductDraft) => {
      ...
      if (packAttribute) {
        ...
        await db.from("product_attribute_terms").delete().eq("product_id", id);
        if (d.termIds.length > 0) {
          await db.from("product_attribute_terms").insert(
      ...
      if (removed.length > 0) {
        await db.from("variations").delete().in("id", removed);
      }
      ...
      for (const [index, v] of d.variations.entries()) {
        ...
        if (v.id) {
          const { error } = await db.from("variations").update(base).eq("id", v.id);
          if (error) throw new Error(error.message);
        } else {
          const { data: createdVariation, error } = await db
            .from("variations")
          ...
          {tab === "General" ? (
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Product name" className="sm:col-span-2">
                <TextInput
                  value={draft.name}
                  placeholder="Groundnut Organic Oil"
                  onChange={(e) =>
                    setDraft({
                      ...draft,
                      name: e.target.value,
              ...
              <Field label="Short description" className="sm:col-span-2">
                <TextInput
                  ...
                  placeholder="Cold-pressed from certified organic seed. Deep, nutty and unrefined."
              ...
              <div className="flex flex-wrap gap-6 sm:col-span-2">
                <Toggle
                  checked={draft.is_organic}
                  onToggle={(v) => setDraft({ ...draft, is_organic: v })}
                  label="Certified organic (shows the Organic badge)"
                />
                <Toggle
