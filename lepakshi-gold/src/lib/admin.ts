  ...
  return queryOptions({
    ...
    queryFn: async (): Promise<{ attributes: AttributeRow[]; terms: AttributeTermRow[] }> => {
      const [a, t] = await Promise.all([
        ...
        supabase.from("attribute_terms").select("*").order("sort_order"),
      ]);
  ...
  return queryOptions({
    ...
    queryFn: async (): Promise<StockRow[]> => {
      const [v, p] = await Promise.all([
        supabase.from("variations").select("*").order("sku"),
        supabase.from("products").select("id,name,slug,category_id,is_ganuga"),
      ]);
      if (v.error) throw new Error(v.error.message);
      if (p.error) throw new Error(p.error.message);
      ...
      return (v.data ?? []).map((row) => {
        ...
        return {
          ...
          is_ganuga: parent?.is_ganuga ?? false,
        };
      });
    },
  });
