...
function HomeBlocks() {
  ...
  if (!hydrated && rows.length > 0) {
    ...
    setHero({
      ...
      secondaryLabel: heroBlock?.["secondaryLabel"] ?? "How ganuga works",
    });
    setAnnouncement(
      annBlock?.["text"] ?? "Wood-pressed the traditional way · Free delivery above ₹999",
    );
    setHydrated(true);
  }
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      for (const row of upserts) {
        ...
        const res = existing
          ? await supabase.from("content_blocks").update(row).eq("id", existing.id)
        ...
        <div className="space-y-4">
          <Field label="Question">
            <TextInput
              ...
              placeholder="Why does ganuga oil cost more than refined oil?"
