...
export const Route = createFileRoute("/shop/")({
  head: () => ({
    meta: [
      { title: "Shop all oils — Lepakshi Gold" },
      {
        name: "description",
        content:
          "Browse every Lepakshi Gold edible oil: groundnut, coconut, sesame, sunflower, safflower, mustard, rice bran and palm.",
      },
      { property: "og:title", content: "Shop all oils — Lepakshi Gold" },
      {
        property: "og:description",
        content: "Wood-pressed and filtered oils in 500 ml to 15 L packs.",
      },
...
function Shop() {
  ...
  const [ganugaOnly, setGanugaOnly] = useState(false);

  const list = useMemo(() => {
    let rows = [...(products.data ?? [])];
    if (ganugaOnly) rows = rows.filter((p) => p.is_ganuga);
    rows.sort((a, b) => {
        ...
        <div className="mt-8 flex flex-wrap items-center gap-4 border-y border-line-200 py-4">
          <label className="flex items-center gap-2 text-sm text-ink-900">
            <input
              type="checkbox"
              checked={ganugaOnly}
              onChange={(e) => setGanugaOnly(e.target.checked)}
              className="size-4 accent-green-900"
            />
            Ganuga (wood-pressed) only
          </label>
